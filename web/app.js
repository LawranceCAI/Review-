...
function legacyWarning(data){
  try{
    ...
    const schema = meta && meta.schema ? meta.schema : "";
    if(schema !== "direction-v1"){
      const el = document.getElementById("legacyWarn");
      if(el){
        el.textContent = "⚠️ 目前 cards.json 不是 direction-v1（可能仍是舊版 qa/cloze）。請確認你上傳的是新版專案，且 Actions 已成功重建 cards.json。";
...
function maskHighlights(text, highlights){
  if(!highlights || !highlights.length) return text;
  let out = text;
  for(const h of highlights){
    if(!h) continue;
    out = out.split(h).join("_____");
  }
  return out;
}
...
async function main(){
  ...
  $("#btnShow").addEventListener("click", () => {
    ...
    if(card.type === "direction"){
      ...
      const hintMax = Math.min(3, blocks.length);
